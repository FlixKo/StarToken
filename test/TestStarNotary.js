...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('Test Stars', tokenId1, {from: accounts[1]});
...
it('lets 2 users exchange stars', async() => {
    ...
    let instance = await StarNotary.deployed();
    let tokenId1 = 135;
    await instance.createStar('Starry Stars', tokenId1, {from: accounts[1]});

    let tokenId2 = 159;
    await instance.createStar('More Stars', tokenId2, {from: accounts[2]});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('Earth', starId, { from: accounts[1] });
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 625;
    let instance = await StarNotary.deployed();
    await instance.createStar('Star4', tokenId, {from: accounts[0]})
    // 2. Call your method lookUptokenIdToStarInfo
    let star = await instance.lookUptokenIdToStarInfo(tokenId);
    // 3. Verify if you Star name is the same
    assert.equal('Star4', star);
});